...
def find_authors_nationalities():
    ...
    result = [f"{authors.first_name} {authors.last_name} is {authors.nationality}" for authors in authors]
    return '\n'.join(result)
...
def order_books_by_year():
    return '\n'.join([f"{book.publication_year} year: {book.title} by {book.author}" for book in
                      Book.objects.order_by('publication_year', 'title')])
...
def delete_review_by_id(r_id):
    person = Review.objects.get(id=r_id)
    person.delete()
...
# def filter_authors_by_nationalities(nationality):
#     return '\n'.join([(f"{author.biography}") if
#                       author.biography else f"{author.first_name} {author.last_name}"
