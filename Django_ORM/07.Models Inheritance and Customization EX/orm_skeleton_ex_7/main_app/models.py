...
class StudentIDField(models.PositiveIntegerField):
    def to_python(self, value):
        try:
            ...
            pass  # This is where we can handle the error.
            # Not required.
...
class Room(models.Model):
    ...
    def clean(self) -> None:
        if self.total_guests > self.capacity:
            raise ValidationError(
                'Total guests are more than the capacity of the room'
            )
...
class BaseReservation(models.Model):
    ...
    def clean(self):
        if self.start_date >= self.end_date:
            raise ValidationError(
                'Start date cannot be after or in the same end date'
            )
        if not self.is_available:
            raise ValidationError(
                f'Room {self.room.number} cannot be reserved'
            )
...
class SpecialReservation(BaseReservation):
    ...
    def extent_reservation(self, days: int):
        # change the property method to add param days
